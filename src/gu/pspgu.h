...
**/
static inline void* sceGuSetCallback(int signal, void (*callback)(int));
...
**/
static inline void sceGuSendList(int mode, const void* list, PspGeContext* context);
...
static inline void drawRegion(int x, int y, int width, int height)
{
	...
	sceGuSendCommandi(REGION2, (((y + height) - 1) << 10) | ((x + width) - 1));
}
...
static inline void* sceGuSetCallback(int signal, GuCallback callback)
{
...
static inline void sceGuLightColor(int light, int component, unsigned int color)
{
	...
	switch (component)
	{
		case GU_AMBIENT: sceGuSendCommandi(settings->ambient, color & 0xffffff); break;
		case GU_DIFFUSE: sceGuSendCommandi(settings->diffuse, color & 0xffffff); break;
		case GU_AMBIENT_AND_DIFFUSE:
		{
			sceGuSendCommandi(settings->ambient, color & 0xffffff); break;
			sceGuSendCommandi(settings->diffuse, color & 0xffffff); break;
		}
		break;
		
		case GU_SPECULAR: sceGuSendCommandi(settings->specular, color & 0xffffff); break;
		case GU_DIFFUSE_AND_SPECULAR:
		{
			sceGuSendCommandi(settings->diffuse, color & 0xffffff); break;
			sceGuSendCommandi(settings->specular, color & 0xffffff); break;
		}
		break;
	}
}
...
static inline void sceGuTexFlush(void)
{
	sceGuSendCommandf(TEX_FLUSH, 0.0f);
}
...
static inline void sceGuTexImage(int mipmap, int width, int height, int tbw, const void *tbp)
{
	GECommand texAddr = TEX_ADDR0 + mipmap;
	GECommand texBufWidth = TEX_BUF_WIDTH0 + mipmap;
	GECommand texSize = TEX_SIZE0 + mipmap;
	sceGuSendCommandi(texAddr, ((unsigned int)tbp) & 0xffffff);
	sceGuSendCommandi(texBufWidth, ((((unsigned int)tbp) >> 8) & 0x0f0000) | tbw);
	sceGuSendCommandi(texSize, (getExp(height) << 8) | (getExp(width)));
	sceGuTexFlush();
}
